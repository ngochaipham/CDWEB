...
import cdw.admin.service.ContractService;
import cdw.admin.service.EmployeeService;
import cdw.admin.validator.ContractValidator;
import cdw.admin.validator.EmployeeValidator;
import cdw.model.*;
import cdw.other.SendMail;
...
public class EmployeeController extends ShopController {
	...
	public String formReset(ModelMap model, @PathVariable(value = "id") String id,
		...
		employee.setPassword("CDWebShop" + new Date().getDate() + new Date().getMonth() + new Date().getYear());
		employeeService.editEmployee(employee);
		SendMail.sendMail(employee.getEmail(), "Xin Chào, Thành Viên: " + employee.getNameEmployee(),
